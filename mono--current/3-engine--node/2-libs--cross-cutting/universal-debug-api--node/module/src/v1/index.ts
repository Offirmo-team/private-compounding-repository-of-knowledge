import { type LogLevel, DEFAULT_LOG_LEVEL, DEFAULT_LOGGER_KEY } from "@monorepo-private/practical-logger--core"
import { type Logger, type LoggerCreationParams, createLogger } from "@monorepo-private/practical-logger--node"
...
export default function create(): DebugApiV1 {
	...
	try {
		if (forcedLevel) _ownLogger.setLevel(JSON.parse(forcedLevel) as LogLevel)
	} catch (err) {
		...
		if (!overrides[key]) {
			...
			if (rawValue) {
				...
				const value = (() => {
					...
					if (isꓽexact_stringified_number(rawValue)) return Number(rawValue)
	...
	function overrideHook<T>(key: string, defaultValue: T): T {
		try {
			...
			if (status.isOn) return status.value as T
		} catch (err) {
