...
import type { LogDetails } from "@monorepo-private/practical-logger--types"
	...
	describe("normalization of arg[0] = message", () => {
		...
		context("on tolerated calls", () => {
			it("should work", () => {
				...
				expect(getNormalizedMessage({ bar: 42, message: "hello" }), "direct details + integrated message").to.equal(
					"hello",
				)
			})
		})
		...
		context("on bad calls", () => {
			it("should work", () => {
				...
				expect(getNormalizedMessage("hello", "world!", { err, bar: 42 }, 42), "<- many args").to.equal(
					"hello world! 42",
				)
				expect(getNormalizedMessage({ bar: 42 }, { baz: 33 }), "<- double details").to.deep.equal("(no message)")
				expect(getNormalizedMessage({ bar: 42 }, { baz: 33 }, err), "<- error anywhere - 1").to.deep.equal(
					"err_msg!",
				)
				expect(getNormalizedMessage({ bar: 42 }, { baz: 33 }, { err }), "<- error anywhere - 2").to.deep.equal(
					"err_msg!",
				)
			})
		})
	...
	describe("normalization of arg[1] = details", () => {
		...
		context("on tolerated calls", () => {
			it("should work", () => {
				...
				expect(
					getNormalizedDetails({ bar: 42, message: "hello" }),
					"direct details + integrated message",
				).to.deep.equal({ bar: 42 })
			})
		})
		...
		context("on bad calls", () => {
			it("should work", () => {
				...
				expect(getNormalizedDetails({ bar: 42 }, { baz: 33 }), "<- double details").to.deep.equal({
					bar: 42,
					baz: 33,
				})
				expect(getNormalizedDetails({ bar: 42 }, { baz: 33 }, err), "<- error anywhere - 1").to.deep.equal({
					bar: 42,
					baz: 33,
					err,
				})
				expect(getNormalizedDetails({ bar: 42 }, { baz: 33 }, { err }), "<- error anywhere - 2").to.deep.equal({
					bar: 42,
					baz: 33,
					err,
				})
