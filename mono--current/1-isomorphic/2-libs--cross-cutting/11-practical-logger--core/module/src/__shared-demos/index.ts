import { ALL_LOG_LEVELS, type Logger } from "@monorepo-private/practical-logger--core"
...
function demo_logger_basic_usage(logger: Logger, in_group = true) {
	...
	logger.log(
		'Test of a very very long message: "In publishing and graphic design, Lorem ipsum is a placeholder text commonly used to demonstrate the visual form of a document or a typeface without relying on meaningful content. Lorem ipsum may be used as a placeholder before final copy is available."',
	)
...
function demo_logger_levels(logger: Logger) {
	...
	;[...ALL_LOG_LEVELS].reverse().forEach((level) => logger[level](`message with level "${level}"`, { level, foo: 42 }))
...
function demo_devtools_fonts() {
	...
	;[
		...
		console.log(`%c${font}: ABCdefi012 %cABCdefi012`, `font-family: ${font};`, "font-family: unset;"),
	)
	console.groupEnd()
}
