...
import { trim_before_stringify } from "./index.ts"

describe("trim_before_stringify()", function () {
	describe("ALREADY SORTED situations", function () {
		function test_against_builtin(value: any): void {
			...
			const resultⵧstable = (() => {
				try {
					return JSON.stringify(trim_before_stringify(value))
				} catch (err) {
					return `<exception! ${(err as any)?.message?.split("\n")?.[0]}>`
				}
			})()
			...
			if (resultⵧbuiltin !== resultⵧstable) {
				console.log("≡")
				console.log("☑ unsorted :", resultⵧbuiltin)
				console.log("☐ sorted   :", resultⵧstable)
			}
		...
		describe("handling of non-primitive type/values", function () {
			...
			describe("objects/hashes", function () {
				...
				it.skip("should work with attributes of non-primitive types (key + value)", () => {
					test_against_builtin({
						foo() {},
						0.2e3: {
							n: 42,
			...
			describe.skip("other object", function () {
				...
				it.skip("should work with objects -- this", () => {
