...
import { isꓽobjectⵧplain } from "../01-primitives/index.ts"
...
function isꓽobjectⵧplainⵧempty(o: Immutable<object>): o is {} {
	if (!isꓽobjectⵧplain(o)) return false // safety against wrong dynamic types

	return isꓽarrayⵧempty(Object.keys(o))
}

function isꓽcontainer(c: Immutable<Array<any> | object>): c is {} | [] {
	return Array.isArray(c) || isꓽobjectⵧplain(c)
}

function isꓽcontainerⵧempty(c: Immutable<Array<any> | object>): c is {} | [] {
	if (Array.isArray(c)) return isꓽarrayⵧempty(c)

	return isꓽobjectⵧplainⵧempty(c)
}
...
export { isꓽarrayⵧempty, isꓽobjectⵧplainⵧempty, isꓽcontainer, isꓽcontainerⵧempty, hasꓽcontent }
