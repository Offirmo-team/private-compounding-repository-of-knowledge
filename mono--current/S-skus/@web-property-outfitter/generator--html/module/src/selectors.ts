...
import type { Url‿str, Css‿str, Html‿str, JS‿str, Contentⳇweb } from "@monorepo-private/ts--types--hypermedia"
...
function getꓽcontent_blocksⵧcssⵧcritical(spec: Immutable<HtmlFileSpec>): Immutable<Css‿str[]> {
	...
	// 3. imports
	// 4. everything else
	const namespaces: Css‿str[] =
		Object.entries(Selectors.getꓽcssⵧtop__namespaces(spec.content)).length === 0
			? []
			: [
					`/* define namespaces for usage in following CSS, needs to be at the top https://github.com/parcel-bundler/parcel/issues/9534 */`,
					...Object.entries(Selectors.getꓽcssⵧtop__namespaces(spec.content)).map(
						([name, url]) => `@namespace ${name} url(${url});`,
					),
				]
	const layer: Css‿str[] =
		Selectors.getꓽcssⵧtop__layers(spec.content).length === 0
			? []
			: [
	...
	return [...namespaces, ...layer, ...imports, ...rest]
}
...
function getꓽcontent_html__root__attributes(
	spec: Immutable<HtmlFileSpec>,
): Immutable<NonNullable<Contentⳇweb["html__root__attributes"]>> {
...
function getꓽmetas(spec: Immutable<HtmlFileSpec>): Immutable<HtmlMetas> {
	...
	const metas: HtmlMetas = {
		...
		pragmas: {
			...
			generator: "@web-property-outfitter/generator--html",
			keywords: [] as string[],
		...
		function _ensure_namespace(name: string, url: Url‿str) {
			...
			if (content_expanded.cssⵧtop__namespaces[name]) {
				...
				assert(
					...
					`Namespace conflict: "${name}" already defined with a different URL!`,
				)
			} else {
				content_expanded.cssⵧtop__namespaces[name] = url
			}
			...
			switch (feature_id) {
				...
				case "normalize-url-trailing-slash": {
					...
					assert(
						...
						`A critical JS snippet should be a named function! (${snippet})}`,
					)
					snippet = `;(${snippet})()`
					content_expanded.jsⵧcritical = [snippet, ...Selectors.getꓽjsⵧcritical(content_expanded)]
					break
				}
				...
				case "cssⳇfoundation--offirmo":
					content_expanded.css = [
						...Selectors.getꓽcss(content_expanded),
		...
		...Object.keys(metas.document)
			...
			.map((name) => {
				...
				assert(
					...
					`document meta "${name}" should be a non-empty string: "${content}"!`,
				)
...
export { getꓽfeatures, getꓽtitleⵧpage, getꓽhtml‿str, getꓽcontent_html__root__attributes }
