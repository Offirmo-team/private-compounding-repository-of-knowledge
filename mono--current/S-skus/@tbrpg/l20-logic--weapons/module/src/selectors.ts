...
Object.keys(OVERALL_STRENGTH_INTERVAL_BY_QUALITY).forEach((k: string): void => {
	...
	if (base_min >= base_max) throw new Error(`${LIB}: range assertion failed for "${quality}"!`)

	TEMP_BASE_STRENGTH_INTERVAL_BY_QUALITY[quality] = [base_min, base_max]
})
const BASE_STRENGTH_INTERVAL_BY_QUALITY: Readonly<{ [k: string]: [number, number] }> =
	TEMP_BASE_STRENGTH_INTERVAL_BY_QUALITY
...
function get_damage_interval(weapon: Immutable<Weapon>): [number, number] {
	return getꓽinterval(weapon.base_strength, weapon.quality, weapon.enhancement_level)
}
...
function getꓽultimate_medium_damage(weapon: Immutable<Weapon>): number {
	const max_damage_range = getꓽinterval(weapon.base_strength, weapon.quality, MAX_ENHANCEMENT_LEVEL)
...
function matches(weapon: Immutable<Weapon>, elements: Immutable<Partial<Weapon>>): boolean {
	...
	if (!weapon) throw new Error(`${LIB} matches: can't match nothing!`)
	...
	;(Object.keys(elements) as Array<keyof Weapon>).forEach((k: keyof Weapon) => {
		if (!(k in weapon)) throw new Error(`${LIB} matches: can't match on non-weapon key "${k}"!`)
