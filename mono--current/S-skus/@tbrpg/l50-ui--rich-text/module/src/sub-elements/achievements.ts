import { AchievementStatus, type AchievementSnapshot } from "@tbrpg/state--achievements"

import * as RichText from "@monorepo-private/rich-text-format"
import type { Immutable } from "@monorepo-private/ts--types"

function renderꓽachievement_snapshotⵧshort(achievement_snapshot: Immutable<AchievementSnapshot>): RichText.Document {
	const { uuid, icon, name, status, completion_rate } = achievement_snapshot

	const builder = RichText.fragmentⵧinline().addClass("achievement", `achievement--${status}`)
	...
	if (icon_text) {
		builder.pushEmoji(icon_text).pushText("  ")
	}

	if (status === AchievementStatus.unlocked) builder.pushStrong(legend)
	else builder.pushWeak(legend)
...
function renderꓽachievement_snapshotⵧdetailed(achievement_snapshot: Immutable<AchievementSnapshot>): RichText.Document {
	const { uuid, icon, name, description, lore, status, completion_rate } = achievement_snapshot
	const element_tags = ["achievement"]

	const builder = RichText.fragmentⵧblock().addClass("achievement")
	...
	builder.pushStrong(name_text, { classes: ["achievement__title"] }).pushHorizontalRule()
	...
	if (lore_text) {
		builder.pushWeak(`“${lore_text}“`, { classes: ["achievement__lore"] }).pushLineBreak()
	}
...
function renderꓽachievements_snapshot(
	ordered_achievement_snapshots: Immutable<AchievementSnapshot>[],
): RichText.Document {
	const builder = RichText.listⵧunordered().addClass("achievements-snapshot")

	ordered_achievement_snapshots.forEach((achievement_snapshot) => {
		const { uuid } = achievement_snapshot
		//console.log(uuid)
		builder.addSub(renderꓽachievement_snapshotⵧshort(achievement_snapshot), { id: uuid })
	})

	const $doc_list = builder.done()

	const stats = ordered_achievement_snapshots.reduce(
		(acc, { status }) => {
			switch (status) {
				case AchievementStatus.secret:
					break
	...
	const $doc = RichText.fragmentⵧblock()
		.setHeading(
			RichText.fragmentⵧinline().pushText(`Achievements (${stats.unlocked_count}/${stats.visible_count})`).done(),
		)
...
export { renderꓽachievement_snapshotⵧshort, renderꓽachievement_snapshotⵧdetailed, renderꓽachievements_snapshot }
