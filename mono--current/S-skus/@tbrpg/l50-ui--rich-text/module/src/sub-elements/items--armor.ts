...
function push_power(
	...
	if (options.reference_power) {
		if (power > options.reference_power) {
			...
			builder.pushSubNode($node, { id: "comparision" })
...
function render_armor_short(
	i: Immutable<Armor>,
	options: Immutable<RenderItemOptions> = DEFAULT_RENDER_ITEM_OPTIONS,
): RichText.Document {
	if (i.slot !== InventorySlot.armor) throw new Error(`render_armor_short(): can't render a ${i.slot}!`)
	...
	builder.pushSubNode(render_armor_name(i), { id: "name" })
...
function render_armor_detailed(i: Immutable<Armor>, reference_power?: number): RichText.Document {
	if (i.slot !== InventorySlot.armor) throw new Error(`render_armor_detailed(): can't render a ${i.slot}!`)
