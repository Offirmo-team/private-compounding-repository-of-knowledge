...
import { CoinsGain, AdventureType, type OutcomeArchetype, type AdventureArchetype } from "./types.ts"

/////////////////////

const ALL_ADVENTURE_ARCHETYPES: Readonly<AdventureArchetype>[] = ENTRIES.map((paa) => {
...
function getꓽarchetype(hid: string): Readonly<AdventureArchetype> {
	...
	if (!aa) throw new Error(`logic-adventures, getꓽarchetype(): couldn't find archetype "${hid}" !`)
	return aa!
}
...
function generate_random_coin_gain_or_loss(
	rng: RNGEngine,
	{
		range,
		player_level,
	...
	switch (range) {
		...
		default: {
			...
			let interval = COINS_GAIN_RANGES[range]!
			assert(interval, `known range "${range}"`)
			interval = [Math.round(interval[0] * level_multiplier), Math.round(interval[1] * level_multiplier)]
