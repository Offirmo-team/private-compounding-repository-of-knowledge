...
describe(`${LIB} - logic`, function () {
	...
	describe("coin gain picker", function () {
		it("should provide an amount proportional to the gain category", () => {
			...
			const clS1 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.lossꘌsmall,
				player_level,
				current_wallet_amount: 100000,
			})
			expect(clS1).to.be.gte(-20)
			expect(clS1).to.be.lte(-1)
			const clS2 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.lossꘌsmall,
				player_level,
				current_wallet_amount: 100000,
			})
			...
			const clO1 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.lossꘌone,
				player_level,
				current_wallet_amount: 100000,
			})
			expect(clO1, "clO1").to.equal(-1)
			const clO2 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.lossꘌone,
				player_level,
				current_wallet_amount: 100000,
			})
			...
			const cgN1 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.none,
				player_level,
				current_wallet_amount: 100000,
			})
			expect(cgN1, "cgN1").to.equal(0)
			const cgN2 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.none,
				player_level,
				current_wallet_amount: 100000,
			})
			...
			const cgO1 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.gainꘌone,
				player_level,
				current_wallet_amount: 100000,
			})
			expect(cgO1, "cgO1").to.equal(1)
			const cgO2 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.gainꘌone,
				player_level,
				current_wallet_amount: 100000,
			})
			...
			const cgS1 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.gainꘌsmall,
				player_level,
				current_wallet_amount: 100000,
			})
			expect(cgS1).to.be.gte(1)
			expect(cgS1).to.be.lte(20)
			const cgS2 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.gainꘌsmall,
				player_level,
				current_wallet_amount: 100000,
			})
			...
			const cgM1 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.gainꘌmedium,
				player_level,
				current_wallet_amount: 100000,
			})
			expect(cgM1).to.be.gte(50)
			expect(cgM1).to.be.lte(100)
			const cgM2 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.gainꘌmedium,
				player_level,
				current_wallet_amount: 100000,
			})
			...
			const cgB1 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.gainꘌbig,
				player_level,
				current_wallet_amount: 100000,
			})
			expect(cgB1).to.be.gte(500)
			expect(cgB1).to.be.lte(700)
			const cgB2 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.gainꘌbig,
				player_level,
				current_wallet_amount: 100000,
			})
			...
			const cgH1 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.gainꘌhuge,
				player_level,
				current_wallet_amount: 100000,
			})
			expect(cgH1).to.be.gte(900)
			expect(cgH1).to.be.lte(2000)
			const cgH2 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.gainꘌhuge,
				player_level,
				current_wallet_amount: 100000,
			})
		...
		it("should cap the loss to the current wallet amount", () => {
			...
			// -1  -6  -2
			const cgL1 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.lossꘌsmall,
				player_level,
				current_wallet_amount: 1,
			})
			expect(cgL1).to.equal(-1)
			const cgL2 = generate_random_coin_gain_or_loss(rng, {
				range: CoinsGain.lossꘌsmall,
				player_level,
				current_wallet_amount: 2,
			})
