...
import { type RawAdventureArchetypeEntry, i18n_messages, ENTRIES } from "./index.ts"
	...
	describe("[distribution]", function () {
		...
		it("has an outcome of each type with a correct distribution", () => {
			...
			ENTRIES_GOOD.forEach((entry) => {
				const stats = DISTRIB_ADJUSTMENT_ENTRIES_HIDS.includes(entry.hid) ? stats_adjustments : stats_normal
				...
				Object.keys(outcome).forEach((gain_key) => {
					if (typeof outcome[gain_key] !== "boolean" && gain_key !== "token") {
						...
						stats[gain_key] = stats[gain_key] ? stats[gain_key] + 1 : 1
					} else {
						stats[gain_key] = stats[gain_key] ? stats[gain_key] + 1 : 1
					}
				})
			...
			expect(Object.keys(stats_normal).sort(), "all seen outcome types").to.deep.equal(
				[...SORTED_EXPECTED_OUTCOMES].sort(),
			)

			SORTED_EXPECTED_OUTCOMES.forEach((gain_key) => {
				let count = stats_normal[gain_key]
				if (stats_adjustments[gain_key]) count += stats_adjustments[gain_key]

				let text = `${gain_key}                `.slice(0, 20) + " = " + count

				if (stats_adjustments[gain_key]) text += ` (${stats_normal[gain_key]} + ${stats_adjustments[gain_key]})`

				console.log(text)
				if (gain_key === "level" || gain_key === "wisdom" || gain_key === "token") console.log("-------")
			})
		})
		...
		it("auto helps to fix the errors", () => {
			...
			if (missing_descriptors.length) {
				missing_descriptors.forEach((key) => {
					...
					console.log(
						`	{ good: true, type: story, ${hid_part.slice(0, 36)} outcome: ${JSON.stringify(outcome)}},`,
					)
				})
			}
		})
