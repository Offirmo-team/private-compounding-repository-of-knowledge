...
import { AchievementStatus, type State } from "./types.ts"
...
function on_played(previous_state: Immutable<State>, details: PlayedDetails): Immutable<State> {
	...
	const { good, adventure_key, encountered_monster_key, active_class, coins_gained, tokens_gained, items_gained } =
		details
...
function on_achieved(previous_state: Immutable<State>, key: string, new_status: AchievementStatus): Immutable<State> {
	...
	if (last_known_status === AchievementStatus.unlocked) {
		...
		assert(
			last_known_status !== AchievementStatus.unlocked,
			`${LIB}: achievements are sticky, they can’t be removed!`,
		)
	}
