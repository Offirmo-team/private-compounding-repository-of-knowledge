...
function get_current_energy_refilling_rate_per_ms(u_state: Readonly<UState>, t_state: Readonly<TState>): Fraction {
	...
	const total_energy_refilled_so_far =
		total_energy_consumed_so_far + getꓽavailable_energy‿int(t_state) - u_state.max_energy
	assert(
		total_energy_refilled_so_far >= 0,
		`${LIB}.get_current_energy_refilling_rate_per_ms() total_energy_refilled_so_far = ${total_energy_refilled_so_far}!`,
	)
	...
	let rate = onboarding_energy_refilling_rate_per_ms
...
function get_milliseconds_to_next(u_state: Readonly<UState>, t_state: Readonly<TState>): number {
	...
	const ttn = new Fraction(1).sub(fractional_available_energy).div(energy_refilling_rate_per_ms).round(0).valueOf()
	...
	if (ttn <= 0) {
		getꓽlogger().log("ms 0!", {
			...
			ttn,
			ttn_f: new Fraction(1).sub(fractional_available_energy).div(energy_refilling_rate_per_ms),
		})
	}
