	...
	describe("get_current_energy_refilling_rate_per_ms()", function () {
		...
		it("should slowly ramp-up for onboarding - demo", () => {
			...
			for (let i = 0; i < 50; ++i) {
				...
				const ttn = new Fraction(1).div(get_current_energy_refilling_rate_per_ms(u_state, t_state)).round(0)
				/*
		...
		it(`should tends towards ${EXPECTED_ESTABLISHED_ENERGY_REFILL_PER_DAY}`, () => {
			...
			expect(get_energy_refill_rate(u_state, t_state).per_day()).to.be.closeTo(
				EXPECTED_ESTABLISHED_ENERGY_REFILL_PER_DAY,
				0.01,
			)
		})
