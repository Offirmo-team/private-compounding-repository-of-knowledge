...
function use_energy(
	...
	if (now_ms < t_state.timestamp_ms) throw new Error(`${LIB}: time went backward! (cheating attempt?)`)
...
function restore_energy(
	...
	if (available_energy.compare(u_state.max_energy) > 0) available_energy = new Fraction(u_state.max_energy)
...
export { create, update_to_now, use_energy, lose_all_energy, restore_energy }
