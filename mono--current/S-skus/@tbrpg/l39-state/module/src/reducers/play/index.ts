...
function play(
	...
	{ now_ms = getꓽUTC_timestamp‿ms(), explicit_adventure_archetype_hid }: PlayParams = {},
): Immutable<State> {
	...
	state = {
		...
		u_state: {
			...
			progress: AchievementsState.on_played(u_state.progress, {
				...
				items_gained:
					(u_state.last_adventure!.gains.armor ? 1 : 0) + (u_state.last_adventure!.gains.weapon ? 1 : 0),
			}),
		},
	}
