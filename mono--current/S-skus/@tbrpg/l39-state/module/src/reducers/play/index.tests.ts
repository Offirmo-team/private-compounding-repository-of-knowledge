...
import { InventorySlot, get_unequipped_item_count, get_equipped_item_count } from "@tbrpg/state--inventory"
import { Currency, get_currency_amount } from "@tbrpg/state--wallet"
import { expect } from "chai"
import * as icepick from "icepick"

import { LIB } from "../../consts.ts"
import { getꓽavailable_energy‿float } from "../../selectors/index.ts"
import { type State } from "../../types.ts"
import { create } from "../index.ts"
import { _lose_all_energy } from "../internal.ts"

import { play } from "./index.ts"
		...
		context("✅  when allowed (the cooldown has passed / enough energy)", function () {
			...
			context("when the adventure is a story", function () {
				describe("the outcome", function () {
					...
					it("should sometime be an item improvement", () => {
						...
						expect(state.u_state.inventory.slotted[InventorySlot.armor]).to.have.deep.property(
							"enhancement_level",
							0,
						)
						expect(state.u_state.inventory.slotted[InventorySlot.weapon]).to.have.deep.property(
							"enhancement_level",
							0,
						)
			...
			context("when the adventure is a fight", function () {
				it("should generate a suitable enemy", () => {
					let state = create()
					state =
						icepick.setIn(state, ["u_state", "avatar", "attributes", "level"], 500) ||
						(() => {
					...
					for (let i = 0; i < 100; ++i) {
						state =
							icepick.setIn(state, ["t_state", "energy", "available_energy"], { n: 7, d: 1 }) ||
							(() => {
								// This is for typechecking the icepick above
						...
						state = play(state)
						if (state.u_state.last_adventure!.hid.startsWith("fight_")) break
					}
			...
			.forEach(({ hid, good }) => {
				describe(`${good ? "✅" : "🚫"}  adventure "${hid}"`, function () {
					it("should be playable", () => {
						...
						if (!good) state = _lose_all_energy(state)
