...
import { CharacterAttribute, increase_stat } from "@tbrpg/state--character"
...
import { appraise_item_value, is_inventory_full } from "../selectors/index.ts"
...
function _auto_make_room(state: Immutable<State>, options: { DEBUG?: boolean } = {}): Immutable<State> {
	const { DEBUG } = options
	if (DEBUG)
		console.log(`  - _auto_make_room()… (inventory holding ${state.u_state.inventory.unslotted.length} items)`)
	...
	if (is_inventory_full(state.u_state)) {
		...
		Array.from(original_unslotted)
			.filter((e: Immutable<Item>) => {
				switch (e.slot) {
					case InventorySlot.armor:
						if (ArmorLib.matches(e as Armor, STARTING_ARMOR_SPEC)) return false
						break
					case InventorySlot.weapon:
						if (WeaponLib.matches(e as Weapon, STARTING_WEAPON_SPEC)) return false
						break
					default:
						break
				}
			...
			.forEach((e: Immutable<Item>) => {
				...
				switch (e.slot) {
					case InventorySlot.armor:
						if (ArmorLib.matches(e as Armor, STARTING_ARMOR_SPEC)) return
						break
					case InventorySlot.weapon:
						if (WeaponLib.matches(e as Weapon, STARTING_WEAPON_SPEC)) return
						break
					default:
						break
				}
		...
		if (DEBUG)
			console.log(
				`    Freed ${freed_count} items, inventory now holding ${state.u_state.inventory.unslotted.length} items.`,
			)
	}
