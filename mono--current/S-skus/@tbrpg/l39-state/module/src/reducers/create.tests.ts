...
import { get_unequipped_item_count, get_equipped_item_count } from "@tbrpg/state--inventory"
...
describe(`${LIB} -- reducers -- create`, function () {
	...
	describe("🆕  initial state", function () {
		it("should be correct", function () {
			...
			expect(Object.keys(state).sort(), "quick root key check").to.deep.equal(
				["ⵙapp_id", "schema_version", "last_user_investment_tms", "u_state", "t_state"].sort(),
			) // because this test should be updated if that changes
