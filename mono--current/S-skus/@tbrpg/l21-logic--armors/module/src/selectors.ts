...
function getꓽdamage_reduction_interval(armor: Immutable<Armor>): [number, number] {
	return getꓽinterval(armor.base_strength, armor.quality, armor.enhancement_level)
}
...
function getꓽultimate_medium_damage_reduction(armor: Immutable<Armor>): number {
	const max_damage_range = getꓽinterval(armor.base_strength, armor.quality, MAX_ENHANCEMENT_LEVEL)
...
function matches(armor: Immutable<Armor>, elements: Immutable<Partial<Armor>>): boolean {
	...
	if (!armor) throw new Error(`${LIB} matches: can't match nothing!`)
	...
	;(Object.keys(elements) as Array<keyof Armor>).forEach((k: keyof Armor) => {
		if (!(k in armor)) throw new Error(`${LIB} matches: can't match on non-armor key "${k}"!`)
