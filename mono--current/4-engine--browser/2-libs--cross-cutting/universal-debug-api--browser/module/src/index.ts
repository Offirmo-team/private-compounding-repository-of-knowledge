import type { DebugApiRoot, DebugApi } from "@monorepo-private/universal-debug-api--types"
...
root.v1 = ((existing) => {
	...
	if (isExistingAPlaceholder) {
		ownLogger.warn(
			"install warning: a placeholder is already present, you may have missed some calls! the true implementation should be imported earlier! Check the order of your scripts/imports!",
		)
	...
	ownLogger.warn(
		"install warning: several true implementation coexists, only the top module should import it. Check your submodules!",
	)
	...
	try {
		...
		if (minVersion !== candidate._!.minor)
			ownLogger.warn(
				`install warning: several true implementation coexists, including an outdated one: "v${minVersion}"!`,
			)
	} catch (err) {
...
// types
export * from "@monorepo-private/universal-debug-api--types"
